...
df<-read.csv("Data/Study2/marten_data2.csv")


# Load pre-fitted model results from previous analysis
# These .Rdata files contain the fitted model objects from "Analysis.R"
fit <- get(load("Study2/Models/fit_MSCR.Rdata")) 
fit_nomem <- get(load("Study2/Models/fit_CTSCR.Rdata"))
...
# for the thesis we run this code with df$id= 1, 2, 7 )
...
plot_ind <- ggplot(ac_density_ind, aes(x = x, y = y, fill = value)) +
  ...
  coord_fixed(xlim = c(325,329), ylim = c(5006, 5010), expand = FALSE) + # change this for other individuals
  scale_fill_gradient(low = "white", high = "black", limits = c(0, 700))+
  labs(
    x = "Easting (km)",
    y = "Northing (km)",
